...
int ESCSignal;                    // Actual PWM signal sent to the ESC
int potAngle;                     // Potentiometer angle in degrees
...
void loop() {
    ...
    CtrlPWM = map(potValue, 0, 4095, 0, 180);  // Map the reading from [0, 4095] to [0, 180]
    potAngle = map(potValue, 0, 4095, 0, 300);  // Map the reading from [0, 4095] to [0, 300] degrees
    ESCSignal = CtrlPWM * 2000 / 180;  // Calculate the PWM signal for the ESC
    ...
    Serial.print(" degrees - ESC Signal: ");
    Serial.println(ESCSignal);
